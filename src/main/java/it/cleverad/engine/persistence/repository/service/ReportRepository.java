...
public interface ReportRepository extends JpaRepository<Report, Long>, JpaSpecificationExecutor<Report> {
    ...
    @Query(nativeQuery = true, value = "WITH w as (Select distinct datadx                                                                          as giorno, " +
            ...
            "                           COALESCE(round(CAST((SUM(dt.leadnumber) / SUM(dt.clicknumber) * 100) AS numeric), 2), " +
            "                                    0)                                                                     as LR, " +
            ...
            "                                          CAST(SUM(nullif(dt.revenue, 0)) AS numeric) * 100, 2), 0)        as marginePC, " +
            ...
            "       COALESCE(round(CAST((SUM(revenue) - SUM(commission)) / SUM(nullif(revenue, 0)) " +
            "           * 100 AS numeric), 2), 0)                                                      as marginePC, " +
            "       COALESCE(round(SUM(commission) / SUM(nullif(impressionnumber, 0)) * 1000, 2), 0)   as ecpm, " +
            "       COALESCE(round(SUM(commission) / SUM(nullif(clicknumber, 0)), 2), 0)               as ecpc, " +
            "       COALESCE(round(CAST(SUM(commission) / SUM(nullif(leadNumber, 0)) AS numeric), 2), 0) as ecpl " +
            "FROM w " +
            "GROUP BY ROLLUP ((giorno, impressionNumber, clickNumber, leadnumber, ctr, lr, commission, revenue, margine, marginePC, " +
            "                  ecpm, ecpc, ecpl)) " +
