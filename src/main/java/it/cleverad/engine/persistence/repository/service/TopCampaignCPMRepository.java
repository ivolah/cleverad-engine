...
import it.cleverad.engine.persistence.model.service.TopCampaignCPM;
...
public interface TopCampaignCPMRepository extends JpaRepository<TopCampaignCPM, Long>, JpaSpecificationExecutor<TopCampaignCPM> {
    ...
    @Query(nativeQuery = true, value =
            "	Select vtcm.campaignid,		"	+
                    ...
                    "	        FROM v_top_campaign_cpm tt		"	+
                    "	        WHERE tt.campaignid = vtcm.campaignid)                                                        as ecpm,		"	+
                    "	       (SELECT COALESCE(SUM(tt.ecpc), 0)		"	+
                    "	        FROM v_top_campaign_cpc tt		"	+
                    "	        WHERE tt.campaignid = vtcm.campaignid)                                                        as ecpc,		"	+
                    "	       (SELECT COALESCE(SUM(tt.ecpl), 0)		"	+
                    "	        FROM v_top_campaign_cpl tt		"	+
                    "	        WHERE tt.campaignid = vtcm.campaignid)                                                        as ecpl		"	+
                    "			"	+
                    "	from v_top_campaign_cpm vtcm		"	+
                    "	         left join v_top_campaign_cpc vtcc on vtcm.campaignid = vtcc.campaignid		"	+
                    "	         left join v_top_campaign_cpl vtcl on vtcc.campaignid = vtcl.campaignid		"	+
                    "	where (		"	+
    ...
    @Query(nativeQuery = true, value = "	Select		" +
            ...
            "			" +
            "	       (SELECT COALESCE(SUM(tt.ecpm), 0)		" +
            "	        FROM v_top_affiliate_cpm tt		" +
            "	        WHERE tt.affiliateid = vtcm.affiliateid)                                                        as ecpm,		" +
            "			" +
            "	       (SELECT COALESCE(SUM(tt.ecpc), 0)		" +
            "	        FROM v_top_affiliate_cpc tt		" +
            "	        WHERE tt.affiliateid = vtcm.affiliateid)                                                        as ecpc,		" +
            "			" +
            "	       (SELECT COALESCE(SUM(tt.ecpl), 0)		" +
            "	        FROM v_top_affiliate_cpl tt		" +
            "	        WHERE tt.affiliateid = vtcm.affiliateid)                                                        as ecpl		" +
            "			" +
            "	from v_top_affiliate_cpm vtcm		" +
            "	         left join v_top_affiliate_cpc vtcc on vtcm.affiliateid = vtcc.affiliateid		" +
            "	         left join v_top_affiliate_cpl vtcl on vtcc.affiliateid = vtcl.affiliateid		" +
            "	where (		" +
            "	        ((:dateFrom < vtcm.date) AND (:dateTo > vtcm.date)) or		" +
            "	        ((:dateFrom < vtcc.date) AND (:dateTo > vtcc.date)) or		" +
            "	        ((:dateFrom < vtcl.date) AND (:dateTo > vtcl.date))		" +
