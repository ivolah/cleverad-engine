...
public interface WidgetTopCPMRepository extends JpaRepository<WidgetTopCPM, Long>, JpaSpecificationExecutor<WidgetTopCPM> {
    ...
    @Query(nativeQuery = true, value =
            "	Select vtcm.campaignid,		" +
                    ...
                    "	        FROM v_widget_top_cpm tt		" +
                    "	        WHERE tt.campaignid = vtcm.campaignid)                                                        as ecpm,		" +
                    "	       (SELECT COALESCE(SUM(tt.ecpc), 0)		" +
                    "	        FROM v_widget_top_cpc tt		" +
                    "	        WHERE tt.campaignid = vtcm.campaignid)                                                        as ecpc,		" +
                    "	       (SELECT COALESCE(SUM(tt.ecpl), 0)		" +
                    "	        FROM v_widget_top_cpl tt		" +
                    "	        WHERE tt.campaignid = vtcm.campaignid)                                                        as ecpl		" +
                    "			" +
                    "	from v_widget_top_cpm vtcm		" +
                    "	         left join v_widget_top_cpc vtcc on vtcm.campaignid = vtcc.campaignid		" +
                    "	         left join v_widget_top_cpl vtcl on vtcc.campaignid = vtcl.campaignid		" +
                    "	where (		" +
    ...
    @Query(nativeQuery = true, value = "	Select		" +
            ...
            "			" +
            "	from v_widget_top_cpm vtcm		" +
            "	         left join v_widget_top_cpc vtcc on vtcm.affiliateid = vtcc.affiliateid		" +
            "	         left join v_widget_top_cpl vtcl on vtcc.affiliateid = vtcl.affiliateid		" +
            "	where (		" +
            "	        ((:dateFrom < vtcm.date) AND (:dateTo > vtcm.date)) or		" +
            "	        ((:dateFrom < vtcc.date) AND (:dateTo > vtcc.date)) or		" +
            "	        ((:dateFrom < vtcl.date) AND (:dateTo > vtcl.date))		" +
